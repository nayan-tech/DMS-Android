    ...
    private fun returnLivestreamResult(
        ...
        if (result.faceLandmarks().isNotEmpty()) {
            ...
            fun getPoint(index: Int): PointF {
                ...
                return PointF(lm.x() * imageWidth, lm.y() * imageHeight)
            }
            ...
            val MOUTH_LEFT = 78
            val MOUTH_RIGHT = 308
            val LEFT_EYE_TOP = 159
            val LEFT_EYE_BOTTOM = 145
            val RIGHT_EYE_TOP = 386
            val RIGHT_EYE_BOTTOM = 374

            // Calculate positions
            val topHead = getPoint(TOP_HEAD)
            val chin = getPoint(CHIN)
            val nose = getPoint(NOSE)
            val mouthTop = getPoint(MOUTH_TOP)
            val mouthBottom = getPoint(MOUTH_BOTTOM)
            val mouthLeft = getPoint(MOUTH_LEFT)
            val mouthRight = getPoint(MOUTH_RIGHT)
            val leftEyeTop = getPoint(LEFT_EYE_TOP)
            val leftEyeBottom = getPoint(LEFT_EYE_BOTTOM)
            val rightEyeTop = getPoint(RIGHT_EYE_TOP)
            val rightEyeBottom = getPoint(RIGHT_EYE_BOTTOM)

            val leftEye = hypot(leftEyeTop.x - leftEyeBottom.x, leftEyeTop.y - leftEyeBottom.y)
            val rightEye = hypot(rightEyeTop.x - rightEyeBottom.x, rightEyeTop.y - rightEyeBottom.y)
            val eyeRatio = (leftEye + rightEye) / 2.0f

            val mouthWidth = hypot(mouthLeft.x - mouthRight.x, mouthLeft.y - mouthRight.y)
            val mouthHeight = hypot(mouthTop.x - mouthBottom.x, mouthTop.y - mouthBottom.y)
            val mouthRatio = abs(mouthWidth - mouthHeight)



            val headAngle = abs(Math.toDegrees(atan2(
                (chin.y - topHead.y).toDouble(),
                (chin.x - topHead.x).toDouble()
            )))


            // Log or pass this data to UI/analytics modules
            Log.d(TAG, "eyeRatio: $eyeRatio, mouthRatio: $mouthRatio, headAngle: $headAngle")

            // Optionally extend ResultBundle or callback with these values
